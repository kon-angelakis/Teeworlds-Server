...
void CGameContext::OnTick()
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++) //Loop over all slots
	{
		if(m_apPlayers[i]) //If id is connected
		{
			m_apPlayers[i]->Tick();
...
void CGameContext::OnClientEnter(int ClientID)
{
	...
	char aBuf[512];
	str_format(aBuf, sizeof(aBuf), "'%s' has hopped on the %s", Server()->ClientName(ClientID), m_pController->GetTeamName(m_apPlayers[ClientID]->GetTeam()));
